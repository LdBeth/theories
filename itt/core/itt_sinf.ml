doc <:doc<
   ...
   The @tt[Itt_sinf] module defines a ``simple'' infinite type. It is
   the dual to the @tt{srec} type.
...
doc <:doc<
   ...
   The @tt{sinf} term defines the infinite type.  The @tt{srecind}
   term defines an induction combinator over elements of the infinite type.
>>
declare sinf{T. 'B['T]}
declare sinfind{'a; p, h. 'g['p; 'h]}
doc docoff
...
dform sinf_df : except_mode[src] :: sinf{T. 'B} =
   szone nu `"{" slot{'T} `"." pushm[0] slot{'B} `"}" popm ezone

dform sinfind_df : except_mode[src] :: sinfind{'a; p, h. 'g} =
   szone pushm[3]
   tt["sinfind"] `"(" slot{'a} `";" hspace
...
>>
prim_rw unfold_sinfind : sinfind{'a; p, h. 'g['p; 'h]} <-->
...
prim sinf_memberEquality {| intro [] |} :
   sequent { <H> >- 'x1 = 'x2 in sinf{T. 'B['T]} } -->
   [wf] sequent { <H> >- "type"{(sinf{T. 'B['T]})} } -->
   sequent { <H> >- 'A['x1] = 'A['x2] in 'B[sinf{T. 'B['T]}] } =
   it
